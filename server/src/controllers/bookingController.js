...
const calculatePrice = async (serviceId, partIds = []) => {
  ...
  const sRes = await db.query('SELECT price FROM services WHERE id = $1', [serviceId]);
  ...
  if (partIds && partIds.length > 0) {
    const pRes = await db.query('SELECT id, name, price FROM spare_parts WHERE id = ANY($1)', [partIds]);
...
export const createBooking = async (req, res, next) => {
  try {
    ...
    const bRes = await db.query(
      `INSERT INTO bookings (user_id, service_id, address, booking_date, total_amount, subtotal, tax_amount, discount_amount, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
      [userId, serviceId, address, bookingDate, pricing.total, pricing.subtotal, pricing.tax, pricing.discount, 'pending']
    );
    ...
    await db.query(
      ...
      [booking.id, userId, pricing.subtotal, pricing.tax, pricing.discount, pricing.total, pricing, 'unpaid']
    );
...
export const bookService = async (req, res, next) => {
  try {
    const { name, phone, address, service_type, preferred_date, preferred_time, notes } = req.body;
    ...
    if (!service) {
      return res.status(404).json({ success: false, error: `Service type '${service_type}' not found` });
    }
    ...
    // 3. Create booking
    const result = await db.query(
      `INSERT INTO bookings (customer_name, customer_phone, address, service_id, booking_date, preferred_time, notes, total_amount, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'pending') RETURNING *`,
      [name, phone, address, service.id, preferred_date, preferred_time, notes, total]
    );
...
export const updateBooking = async (req, res, next) => {
  try {
    ...
    const currentRes = await db.query('SELECT status FROM bookings WHERE id = $1', [id]);
